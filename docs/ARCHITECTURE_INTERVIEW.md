# Senta Production Deployment Architecture (Redesigned)
...
flowchart LR
    ...
    %% =========================
    subgraph PROD["AWS Cloud - London (eu-west-2) - Production"]
        direction LR

        IGW[Internet Gateway]
        Router[Router]

        subgraph VPC["VPC - application-production-darwinningiris.io"]
            direction LR
            ...
            subgraph AZ1["AZ - eu-west-2a"]
                direction TB
                subgraph AZ1_PRIV["Private Subnet"]
                    EKS1[EKS Worker Nodes<br/>M5]
                    VPCE1[VPC Endpoints<br/>Events / S3 / SNS]
                end
                subgraph AZ1_DB["Database - Private Subnet"]
                    DB1[Aurora PostgreSQL]
                end
                subgraph AZ1_PUB["Public Subnet"]
                    NAT1[NAT Gateway]
                end
            end
            ...
            subgraph AZ2["AZ - eu-west-2b"]
                direction TB
                subgraph AZ2_PRIV["Private Subnet"]
                    EKS2[EKS Worker Nodes<br/>M5]
                    VPCE2[VPC Endpoints<br/>Events / S3 / SNS]
                end
                subgraph AZ2_DB["Database - Private Subnet"]
                    DB2[Aurora PostgreSQL]
                end
                subgraph AZ2_PUB["Public Subnet"]
                    NAT2[NAT Gateway]
                end
            end
            ...
            subgraph AZ3["AZ - eu-west-2c"]
                direction TB
                subgraph AZ3_PRIV["Private Subnet"]
                    EKS3[EKS Worker Nodes<br/>M5]
                    VPCE3[VPC Endpoints<br/>Events / S3 / SNS]
                end
                subgraph AZ3_DB["Database - Private Subnet"]
                    DB3[Aurora PostgreSQL]
                end
                subgraph AZ3_PUB["Public Subnet"]
                    NAT3[NAT Gateway]
                end
            end
        end

        EKSCTRL[Amazon EKS Control Plane]
        CW[CloudWatch]
    end
    ...
    %% Build / CI-CD Account
    %% =========================
    subgraph BUILD["AWS Cloud - Build System Account - cluster.build.ngiris.io (Ireland)"]
        direction TB

        subgraph K8SBUILD["K8s Build Platform"]
            direction LR
            T2A[T2]
            T2B[T2]
            T2C[T2]
            LinuxAgents[Linux Agents<br/>Docker / k8s pods]
            CodeArtifact[AWS CodeArtifact]
            TeamCity[TeamCity]
        end

        ECR[ECR]
        WinAgents[Windows Agents<br/>for EC2 build]
    end

    %% =========================
    %% Tools / SCM
    %% =========================
    Bitbucket[Bitbucket<br/>Source Code]
    SonarQube[SonarQube]
    Snyk[Snyk]
    ...
    TeamCity --> LinuxAgents
    LinuxAgents --> CodeArtifact
    LinuxAgents --> ECR
